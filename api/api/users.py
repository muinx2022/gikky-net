...
def xem_ho_so(request, username: str, limit: int = SO_MACH_TREN_HO_SO):
    ...
    **Tác giả TỰ xoá bình luận thì `duoc_trich` KHÔNG tụt**, và đó là chỗ con số này cố ý
    ...
    return HoSoOut(
        ...
        #
        # KHÔNG dùng lại `doc_duoc` ở trên dù trông gần giống: bộ lọc dưới đây cố ý
        # **thiếu `comment__deleted_at`**. Bốn cột này là đúng bốn cửa làm khối trích biến
        # mất khỏi thẻ mốc (`trich_ra` cho `comment__hidden_at`; `moc_ra` cho `moc__*`;
        # `_mach_hien` cho `moc__mach__hidden_at`) — không thừa, không thiếu. Thêm
        # `comment__deleted_at` vào là chỉ số tụt trong khi blockquote vẫn còn nguyên chữ
        # trên trang mạch; xem docstring endpoint.
        duoc_trich=(
            Trich.objects.filter(
