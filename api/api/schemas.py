...
class TinNhanOut(Schema):
    ...
    body: str
    created_at: datetime
    #: `true` khi tin do CHÍNH người đang gọi gửi.
    cua_toi: bool


class HoiThoaiOut(Schema):
    ...
    tin_cuoi: TinNhanOut | None
    #: Số tin phía kia gửi mà người đang gọi **chưa đọc** trong hội thoại này.
    so_chua_doc: int
    #: `created_at` của tin cuối — khoá sắp của hộp thư (mới nhất trước).
    cap_nhat_luc: datetime


class HopThuOut(Schema):
    ...
    items: list[HoiThoaiOut]
    so_chua_doc: int


class SoChuaDocOut(Schema):
...
class HoiThoaiChiTietOut(Schema):
    ...
    #: `null` khi hai người chưa từng nhắn nhau.
    hoi_thoai_id: int | None
    nguoi_kia: NguoiDungTomTatOut
    items: list[TinNhanOut]
    #: Còn tin cũ hơn `items[0]` không — nút "Tải tin cũ hơn".
    con_cu_hon: bool
