...
| `POST /dem-luot-xem` | ghi MỘT lượt xem trang | *(2026-08-27)* **Không phải cửa cho trình duyệt.** Người gọi duy nhất là `apps/web/middleware.ts` (server-to-server), khoá bằng header secret `X-Dem-Luot-Xem-Secret`: không header ⇒ 401 `chua_dang_nhap`, sai ⇒ 401 `sai_secret`, **secret rỗng ở server ⇒ 503 và không ghi gì** (mặc định của máy dev — fail-closed cả hai đầu). Thân `{duong_dan, user_agent}`; **`user_agent` chỉ để phân loại bot, KHÔNG được lưu**, và **không có cột IP, không cookie** ⇒ không gì trong hai bảng gắn được với một con người. `duong_dan` bị cắt query + bỏ `/` cuối + cắt 200 ký tự. ⚠ Đường mang khoá bí mật (`/dat-lai-mat-khau/{key}`, `/xac-thuc-email/{key}`) **không bao giờ được đếm** — có chuông đọc thẳng `HEADLESS_FRONTEND_URLS` của Django |
...
| `GET /admin/luot-xem` | số liệu trang `/luot-xem` | *(2026-08-27)* `?khoang=7\|30\|90\|tat_ca`. `7/30/90` đọc hàng thô; `tat_ca` = `TongNgay` **+ hàng thô từ `max(TongNgay.ngay)+1`** — mốc lấy từ chính `TongNgay` chứ không phải "hôm nay", để con số **tự lành** khi cron gộp trễ. Bảng bot theo **đúng khoảng đang xem**, và chỉ dựng được từ hàng thô ⇒ cờ `bot_chi_90_ngay` cho màn hình nói ra giới hạn. Bốn con số lớn tính từ nguồn đầy đủ, **không** cộng từ biểu đồ (biểu đồ bị chặn số ô). Chỉ đọc ⇒ mod thường xem được |
