...
| `POST /machs/{id}/tat-binh-luan` | tắt / mở lại bình luận của mạch | *(2026-09-07)* **chỉ author**; thân `{tat: bool}`; mạch bị mod khoá ⇒ 403 `mach_bi_khoa` |
...
| `GET /admin/mocs/{id}` | một mốc để mod SỬA — không che nội dung đã ẩn/bia mộ | *(2026-09-03)* mọi mod đọc được. `sua_duoc` tính ở SERVER (`doc_duoc(moc) and mach.locked_at is None`), frontend không dựng lại luật che. `anhs` liệt kê đủ kể cả ảnh đang cách ly — nhưng **URL của chúng 404** (dựng từ kho đang phục vụ), đó là cái giá đã biết chứ không phải sót |
| `PATCH /admin/mocs/{id}` | mod sửa **đúng 5 trường** của 5.2 trên mốc của NGƯỜI KHÁC | *(2026-09-03)* **chỉ superuser** — sửa lời người khác mạnh hơn ẩn (ẩn = gỡ, sửa = viết lại), cùng nấc với cấp quyền mod. **Không có cửa sổ im lặng 15 phút**: mỗi lượt có đổi để lại `MocRevision` + nhãn «đã sửa» + một dòng `AuditLog` trong CÙNG transaction. Gửi đúng thứ đang có ⇒ `da_doi=false`, không revision, không log. ⚠ Kéo theo một sửa ở `core/ghi.py::sua_moc`: cửa sổ im lặng của TÁC GIẢ nay tắt khi `edited_at` khác `null` — không thì bản của mod bị tác giả ghi đè im lặng trong 15 phút đầu |
| `PATCH /admin/machs/{id}/tieu-de` | sửa tiêu đề mạch (đổi cả `slug`) | *(2026-09-03)* **chỉ superuser**; cửa DUY NHẤT sửa được `Mach.title` — tác giả cũng không (chưa có `MachRevision` nên sửa tiêu đề không để vết được). Đường là `/tieu-de` chứ không phải `PATCH /machs/{id}`: `GET /machs/{id}` ở router khác và resolver lấy pattern khớp đầu ⇒ 405. Làm mới ISR **cả đường cũ lẫn đường mới**, thiếu vế cũ thì slug cũ phục vụ tiêu đề cũ tới một giờ, HTTP 200, không log |
| `POST /admin/anh` · `POST /admin/mocs/{id}/anh` · `DELETE /admin/anh/{id}` | ảnh nhúng trong thân · ảnh đính kèm · gỡ ảnh | *(2026-09-03)* **chỉ superuser**; cùng bảy phép kiểm và cùng kho với v1. Khác v1 hai chỗ: ảnh nhúng **không** chịu hạn mức 30/ngày (cửa này superuser-only), và thêm/gỡ ảnh đính kèm **ghi `AuditLog`** (đổi nội dung người khác). Gắn ảnh vào mốc của mạch đang bị ẩn ⇒ file đi thẳng vào **kho cách ly** (`dong_bo_kho_anh` trong cùng transaction) — thiếu vế đó thì ảnh mới trả 200 trong khi ảnh cũ cùng mốc trả 404 |
