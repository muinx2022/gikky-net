...
export default function TagPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const fetchPosts = useCallback(async (targetPage: number, append: boolean) => {
    ...
    const normalized: Post[] = fetched.map((post) => {
      ...
      return {
        ...post,
        likesCount: typeof s?.likes === "number" ? s.likes : 0,
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h1 className="text-xl font-bold text-slate-900">
                    {tagName}
                    <span className="ml-2 text-sm font-normal text-slate-400">{totalPosts} posts</span>
                  </h1>
                </div>
              </div>
            </div>
              ...
              <div className="divide-y divide-slate-200">
                ...
                <div ref={sentinelRef} className="py-3 text-center">
                  ...
                  {!hasMore && posts.length > 0 && (
                    <span className="text-xs text-slate-400">You have reached the end.</span>
                  )}
                </div>
              </div>
            )}
